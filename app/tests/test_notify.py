...
from src.notify import notify_backup_result


def test_notify_noop_when_url_unset(monkeypatch):
    monkeypatch.setattr(config, "NOTIFY_WEBHOOK_URL", "")

    with patch("src.notify.urllib.request.urlopen") as mock_urlopen:
...
def test_notify_logs_webhook_errors(monkeypatch, caplog):
    ...
    with patch("src.notify.urllib.request.urlopen", side_effect=OSError("network down")):
